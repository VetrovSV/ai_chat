...
def get_context(user_request: str, db, top):
    ...
    """
    # Запрашиваем дополнительный контекст для RAG
    context = db.similarity_search_with_score(user_request, k=top)
    print(context)
    # Берём ближайший вектор
...
def get_Answer_from_YAGPT(text):
    """Реализация работы с YandexGPT"""
    global DB
    # Получаем состояние работы, контекс и ссылки
    state, context, links = get_context(text, DB, top=2)
    # Нулевое состояние, значит прямого ответа нет, нужно сделать запрос к модели
    if state == 0:
        # Запрос к YandexGPT Pro
        req = {
    ...
    # Первое состояние, значит прямого ответа есть и можно выдать сразу его, чтобы не занимать время на обработку моделью
    elif state == 1:
        return context, links
    # Второе состояние, значит ответа в базе знаний нет и стоит ответить, что ответа нет
    elif state == 2:
